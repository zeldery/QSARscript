...
f.close()
chromo = pd.read_csv(result +'/' + 'GAD1'+cls_list[cls_ind]+'-'+str(outp_ind)+str(gamma_ind)+str(C_ind) + str(times)+'.csv',index_col = 0)
chromo = chromo.values

inp_train = pd.read_csv(source + '/D1'+cls_list[cls_ind]+'Tr.csv',index_col = 0)
outp_train = pd.read_csv(source + '/'+cls_list[cls_ind]+'D1Tr.csv',index_col = 0)
inp_test = pd.read_csv(source + '/D1'+cls_list[cls_ind]+'Ts.csv',index_col = 0)
outp_test = pd.read_csv(source + '/'+cls_list[cls_ind]+'D1Ts.csv',index_col = 0)
dat = pd.DataFrame(columns = ['class','output','gamma','C','times','chromosome','R2','Q2CV','Q2'])
current = 0
...
for chromo_ind in range(30):
    ...
    for C in np.arange(C_start[C_ind],C_end[C_ind],C_step[C_ind]):
        for gamma in np.arange(gamma_start[gamma_ind],gamma_end[gamma_ind],gamma_step[gamma_ind]):
            ...
            dat.loc[current,'class'] = cls_list[cls_ind]
