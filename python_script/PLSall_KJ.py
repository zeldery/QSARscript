...
EEVA_sigma = [50]  # [25,50,75,100,150,200]
EEVA_L = [200]     #[25,50,80,100,200] 
EVA_sigma = [1,2,4,6,10,15]
EVA_L = [1,2,5,8,10,20]

dat = pd.DataFrame(columns = ['Output','Components','EEVA sigma','EEVA L','EVA sigma','EVA L','Train','Test'])
current = 0
            ...
            for eva_L in EVA_L:
                ...
                for n_comp in [5,10,20]:
                    ...
                    for outp_index in [0,1,2,4]:
                        ...
                        index = Kennard_Jones(x,int(x.shape[0]*0.375))
                        x_train = x.iloc[index,:]
                        y_train = y.iloc[index,:]
                        x_test = x.drop(x.index[index])
                        y_test = y.drop(y.index[index])
                        ...
                        dat.ix[current,6] = model.score(x_train,y_train.iloc[:,outp_index])
                        dat.ix[current,7] = model.score(x_test,y_test.iloc[:,outp_index])
                        current += 1
        dat.to_csv('final4.csv')